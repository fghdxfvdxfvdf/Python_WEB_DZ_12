...
from src.schemas import ContactModel, ContactResponse, ContactUpdatedModel
...
@router.patch(
    "/{contact_id}/vaccinated",
    response_model=ContactResponse,
    dependencies=[Depends(allowed_operation_update)],
)
async def vaccinated_contact(
    body: ContactUpdatedModel,
    contact_id: int = Path(ge=1),
    db: Session = Depends(get_db),
    current_user: User = Depends(auth_service.get_current_user),
):
    contact = await repository_contacts.set_updated(contact_id, body, db)
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Not Found")
    return contact
